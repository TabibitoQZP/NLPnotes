...
需要注意的是, 这里的标注文本分解成token后可能不止一个, 比如`R & D`会被分解成`R`, `&`和`D`. 此时对应的ner标签索引也会有所变化, `R`会是`R & D`对应原有标签索引, 而`&`和`D`对应的则是-100! 这一点非常重要, 因为pytorch做CrossEncropyLoss会把实际标签为-100的忽略! 这样就不会对这些分类做loss计算.
